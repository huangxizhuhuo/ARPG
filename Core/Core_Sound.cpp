...
bool CAVI::Fast()
{
	...
	m_pMediaPosition->put_Rate(rate*1.5f);
	return true;
}
...
bool CAVI::Slow()
{
	double rate;
	m_pMediaPosition->get_Rate(&rate);
	m_pMediaPosition->put_Rate(rate*0.5f);
	return true;
}
...
{
	m_pMediaControl->Stop();
	m_pMediaPosition->put_CurrentPosition(250);
	m_pMediaControl->Run();
	::Sleep(2);
	return true;
}
bool CAVI::Render(char *FileName)
{
	m_pMediaEvent->SetNotifyWindow((OAHWND)m_hWnd,WM_GRAPH_NOTIFY,NULL);//register a window to process event notifications
	m_pMediaPosition->get_Duration(&m_dLength);
	WCHAR wFileName[MAX_PATH];
	//mbstowcs_s(wFileName,FileName,MAX_PATH);
	size_t len = strlen(FileName) + 1;
	size_t converted = 0;
	mbstowcs_s(&converted,wFileName, len, FileName, MAX_PATH);
...
BOOL CMP3::Init(HWND hWnd)
{
    ...
    if(FAILED(CoCreateInstance(CLSID_FilterGraph, NULL,CLSCTX_INPROC_SERVER,
		IID_IGraphBuilder,(void**)&m_pDSGraphBuilder)))                                          							 
		return FALSE;
    m_pDSGraphBuilder->QueryInterface(IID_IMediaControl,(void**)&m_pDSMediaControl);                                
    m_pDSGraphBuilder->QueryInterface(IID_IMediaEventEx,(void**)&m_pDSMediaEvent);           
    return TRUE;
}
...
BOOL CMP3::Render(char *Filename)
{
    ...
    if(m_pDSGraphBuilder == NULL || m_pDSMediaControl == NULL)
        return FALSE;
    m_pDSMediaEvent->SetNotifyWindow((OAHWND)m_hWnd,WM_GRAPH_NOTIFY,NULL);
