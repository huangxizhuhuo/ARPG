...
{ 
    ...
    CGameObject Object;           // Character object class
    CMesh   WeaponMesh;       // Weapon mesh
    CGameObject WeaponObject;     // Weapon object

    int  MotionNum;//动画在列表中的起始编号,0怒6惊21疑
    int  AniNum;//播放动画副数
    int  FrameNum;//播放速度,若干帧一幅
...
private:
    ...
    CGameFont      *m_Font;                // Font object to use
    ...
    virtual BOOL ActionSound(sCharacter *Character){ return TRUE; }
            
    // Move function for player characters (need to override)
    virtual BOOL PCUpdate(sCharacter *Character, long Elapsed,float *XMove, float *YMove, float *ZMove){ return TRUE; }                                    
                                                      
    // Character update function for all non-PC characters
    BOOL CharUpdate(sCharacter *Character, long Elapsed,float *XMove, float *YMove, float *ZMove);      
                    
    // Check for valid movements. Bounds check to other characters and call ValidateMove (overidden).
    BOOL CheckMove(sCharacter *Character,float *XMove, float *YMove, float *ZMove);                     
                   
    // Virtual ValidateMove for outside bounds checking against character movements.
    virtual BOOL ValidateMove(sCharacter *Character,float *XMove, float *YMove, float *ZMove){ return TRUE; }          
                                      
    // Finish movement by setting direction, animation, etc
    BOOL ProcessUpdate(sCharacter *Character,float XMove, float YMove, float ZMove);                 
                   
    // Process death of a player character
    BOOL PCDeath(sCharacter *Character);              
                 
    // Process death of a non-player character
    virtual BOOL NPCDeath(sCharacter *Character){ return TRUE; }                             

    // Functions to drop money and item when character dies
    virtual BOOL DropMoney(float XPos,float YPos,float ZPos,long Quantity){ return TRUE; }  
                                         
    virtual BOOL DropItem(float XPos, float YPos, float ZPos,long ItemNum,long Quantity){ return TRUE; } 
...
public:
    ...
    BOOL Init(CGraphics *Graphics, CGameFont *Font,char *DefinitionFile, sItem *MIL, sSpell *MSL,
		long NumCharacterMeshes, char **MeshNames,char *MeshPath, char *TexturePath,                                                                                       
		long NumAnimations, sCharAnimationInfo *Anims,CSpellController *SpellController);  
    ...
    BOOL Add(long IDNum, long Definition, long Type, long AI,float XPos, float YPos, float ZPos,float Direction = 0.0f); 
    ...
    BOOL Render(long Elapsed = -1,CFrustum  *Frustum = NULL,float ZDistance = 0.0f);                      
    ...
    virtual BOOL LineOfSight(sCharacter *Source, sCharacter *Target,
		float SourceX, float SourceY, float SourceZ,                                                                                     
		float TargetX, float TargetY, float TargetZ){ return TRUE; }
	...
	long GetHP(sCharacter *Character);
	long GetMP(sCharacter *Character);
    ...
    // Set route points
    BOOL SetRoute(long IDNum,long NumPoints, sRoutePoint *Route);                                 
    ...
    BOOL GetPosition(long IDNum,float *XPos, float *YPos, float *ZPos);                              
                     
    // Functions to Set/Get character bounds
    BOOL SetBounds(long IDNum,float MinX, float MinY, float MinZ,float MaxX, float MaxY, float MaxZ);                                
                                            
    BOOL GetBounds(long IDNum,float *MinX, float *MinY, float *MinZ,float *MaxX, float *MaxY, float *MaxZ);                                
    ...
    BOOL SetMessage(sCharacter *Character, char *Text,long Timer, D3DCOLOR Color=0xFFFFFFFF);        
                
	bool SetEmotion(sCharacter *Character,int StartNum,int PicNum,long Timer,int FrameNum);

    // Process attack damage from spells and physical attacks
    BOOL Damage(sCharacter *Victim,BOOL PhysicalAttack, long Amount,long DmgClass, long CureClass);                           
                                          
    // Process death of an NPC/Monster
    BOOL Death(sCharacter *Attacker, sCharacter *Victim);

    // Process experience up
    virtual BOOL Experience(sCharacter *Character,long Amount){ Character->Def.Experience += Amount; return TRUE; }             
    ...
    BOOL Spell(sCharacter *Caster,sSpellTracker *SpellTracker, sSpell *Spells);                            
                
    // Apply spell effects
    BOOL SpellEffect(sCharacter *Caster, sCharacter *Target,sSpell *Spell);                    

    // Process equipping/unequipping of item
    BOOL Equip(sCharacter *Character, long ItemNum,long Type, BOOL Equip);           
               
    // Process item being used on character
    BOOL Item(sCharacter *Owner, sCharacter *Target,long ItemNum, sCharItem *CharItem = NULL);          
              
    // Process dropping an item
    BOOL Drop(sCharacter *Character,sCharItem *Item, long Quantity);                          
             
    // Process a PC teleport spell
    virtual BOOL PCTeleport(sCharacter *Character,sSpell *Spell){ return TRUE; }             
                                                          
    // Set action (w/timer)
    BOOL SetAction(sCharacter *Character,long Action, long AddTime = 0);                                       
};
#endif
