...
BOOL CFrustum::Construct(CGraphics *Graphics, float ZDistance)
{
    ...
    float      ZMin, Q;
    if(Graphics == NULL)// Error checking
        return FALSE;
    // Calculate FOV data
...
{ 
	short i;
	for(i=0;i<6;i++)// Make sure point is in frustum 
	{  
...
{  
	short i;
	DWORD TotalIn = 0;	
	for(i=0;i<6;i++)// Count the number of points inside the frustum 
	{  
		DWORD Count = 8; 
		BOOL  PointIn = TRUE;    
		...
		if(Count == 0)// If none contained, return FALSE    
			return FALSE;
		TotalIn += (PointIn == TRUE) ? 1:0;// Update counter if they were all in front of plane
	} 	 
	if(CompletelyContained != NULL)// Store BOOL flag if completely contained  
...
{
	short i; 
	for(i=0;i<6;i++)// Make sure radius is in frustum 
	{  
